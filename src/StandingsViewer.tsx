...
const StandingsViewer: React.FC<Props> = ({ equipos = [], partidos = [], onClose, categoria }) => {
    ...
    const compartirTabla = useCallback(async () => {
        ...
        try {
            // ═══════════════════════════════════════════════════════
            //  TABLA DE POSICIONES — épica vertical para WhatsApp
            //  Fondo oscuro, resplandor azul, zona de playoff resaltada,
            //  grupos separados por color. Columnas: JG, JP, DIF, PTS.
            // ═══════════════════════════════════════════════════════
            ...
            const grupos = [
                grupoA.length > 0 ? { teams: grupoA, label: getGroupLabel('A'), color: '#1d4ed8' } : null,
            ...
            const totalRows = grupos.reduce((n, g) => n + g.teams.length, 0);
            const H = HEADER_H
                + grupos.length * (GROUP_H + COLHEAD_H + GROUP_GAP)
                + totalRows * ROW_H + FOOTER_H + 20;
            ...
            const glow = ctx.createRadialGradient(W/2, 40, 0, W/2, 40, 340);
            glow.addColorStop(0, 'rgba(29,78,216,0.33)');
            glow.addColorStop(1, 'transparent');
            ctx.fillStyle = glow; ctx.fillRect(0, 0, W, 220);
            ctx.fillStyle = 'rgba(255,255,255,0.02)';
            for (let x = 0; x < W; x += 20) for (let y = 0; y < H; y += 20) ctx.fillRect(x, y, 2, 2);

            // ── Encabezado ──
            const ligaImg = await loadImg(LIGA_LOGO);
            const logoR = 26, logoCX = W/2, logoCY = 40;
            ctx.save();
            ctx.beginPath(); ctx.arc(logoCX, logoCY, logoR, 0, Math.PI*2);
            ctx.fillStyle = 'white'; ctx.fill(); ctx.clip();
            if (ligaImg) ctx.drawImage(ligaImg, logoCX-logoR, logoCY-logoR, logoR*2, logoR*2);
            ctx.restore();
            ctx.beginPath(); ctx.arc(logoCX, logoCY, logoR, 0, Math.PI*2);
            ...
            ctx.font = '600 12px system-ui'; ctx.fillStyle = 'rgba(255,255,255,0.5)';
            espaciar('LIGA METROPOLITANA · ' + categoria.toUpperCase(), 1, 92);
            ctx.font = '800 28px system-ui'; ctx.fillStyle = 'white';
            ctx.fillText('TABLA DE POSICIONES', W/2, 122);
            ctx.font = '700 11px system-ui'; ctx.fillStyle = '#60a5fa';
            espaciar('FASE REGULAR', 5, 140);

            // ── Preload logos de equipos ──
            ...
            const innerW = W - M*2;
            // posiciones de columnas (derecha del bloque)
            const xJG  = W - 218;
            const xJP  = W - 162;
            const xDIF = W - 100;
            const xPTS = W - 40;
            ...
            for (const grupo of grupos) {
                ...
                ctx.beginPath(); ctx.roundRect(M, y, innerW, GROUP_H, [10,10,0,0]); ctx.fill();
                ctx.font = '700 14px system-ui'; ctx.fillStyle = 'white'; ctx.textAlign = 'left';
                ctx.fillText(grupo.label.toUpperCase(), M + 16, y + 26);
                y += GROUP_H;

                // fila de encabezado de columnas
                ctx.fillStyle = '#0d1424'; ctx.fillRect(M, y, innerW, COLHEAD_H);
                ctx.font = '600 10px system-ui'; ctx.fillStyle = '#475569';
                ctx.textAlign = 'left'; ctx.fillText('EQUIPO', M + 62, y + 20);
                ctx.textAlign = 'center';
                ctx.fillText('JG',  xJG,  y + 20);
                ctx.fillText('JP',  xJP,  y + 20);
                ctx.fillText('DIF', xDIF, y + 20);
                ctx.fillText('PTS', xPTS, y + 20);
                y += COLHEAD_H;
                ...
                for (let i = 0; i < grupo.teams.length; i++) {
                    ...
                    // fondo de fila — resaltado si zona de playoff
                    if (eq.playoffZone) {
                        const g2 = ctx.createLinearGradient(M, 0, W*0.7, 0);
                        g2.addColorStop(0, grupo.color + '2e');
                        g2.addColorStop(1, 'transparent');
                        ctx.fillStyle = g2; ctx.fillRect(M, y, innerW, ROW_H);
                        ctx.fillStyle = grupo.color; ctx.fillRect(M, y, 4, ROW_H);
                    } else {
                        ctx.fillStyle = i % 2 ? 'rgba(255,255,255,0.015)' : 'transparent';
                        ctx.fillRect(M, y, innerW, ROW_H);
                    }

                    // posición
                    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
                    ctx.font = '800 22px system-ui';
                    ctx.fillStyle = i === 0 ? '#fbbf24' : eq.playoffZone ? '#cbd5e1' : '#64748b';
                    ctx.fillText(String(i + 1), M + 24, cy);

                    // logo del equipo
                    const logoImg = eq.logoUrl ? (logoCache[eq.logoUrl] ?? null) : null;
                    const lcx = M + 58;
                    ctx.save();
                    ctx.beginPath(); ctx.arc(lcx, cy, 17, 0, Math.PI*2);
                    if (logoImg) {
                        ctx.fillStyle = 'white'; ctx.fill(); ctx.clip();
                        ctx.drawImage(logoImg, lcx-17, cy-17, 34, 34);
                    } else {
                        ctx.fillStyle = '#1e293b'; ctx.fill(); ctx.clip();
                        ctx.font = '700 12px system-ui'; ctx.fillStyle = '#64748b';
                        ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
                        const ini = eq.nombre.trim().slice(0,3).toUpperCase();
                        ctx.fillText(ini, lcx, cy + 1);
                    }
                    ctx.restore();

                    // nombre + etiqueta playoff
                    ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
                    let nombre = eq.nombre.toUpperCase();
                    ctx.font = '700 15px system-ui';
                    const maxNameW = xJG - (M + 84) - 20;
                    while (nombre.length > 1 && ctx.measureText(nombre).width > maxNameW) nombre = nombre.slice(0, -1);
                    if (nombre.length < eq.nombre.length) nombre = nombre.trimEnd() + '…';
                    ctx.fillStyle = eq.playoffZone ? 'white' : '#94a3b8';
                    ctx.fillText(nombre, M + 84, eq.playoffZone ? cy - 3 : cy + 5);
                    if (eq.playoffZone) {
                        ctx.font = '700 9px system-ui'; ctx.fillStyle = '#10b981';
                        ctx.fillText('✓ CLASIFICA A PLAYOFF', M + 84, cy + 13);
                    }

                    // stats
                    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
                    const dif = eq.dif > 0 ? '+' + eq.dif : String(eq.dif);
                    ctx.font = '700 16px system-ui'; ctx.fillStyle = '#10b981'; ctx.fillText(String(eq.victorias), xJG, cy);
                    ctx.fillStyle = '#94a3b8'; ctx.fillText(String(eq.derrotas), xJP, cy);
                    ctx.font = '700 15px system-ui'; ctx.fillStyle = eq.dif >= 0 ? '#60a5fa' : '#ef4444'; ctx.fillText(dif, xDIF, cy);
                    ctx.font = '800 21px system-ui'; ctx.fillStyle = i === 0 ? '#fbbf24' : 'white'; ctx.fillText(String(eq.puntos), xPTS, cy);

                    // separador
                    ctx.strokeStyle = 'rgba(255,255,255,0.05)'; ctx.lineWidth = 1;
                    ctx.beginPath(); ctx.moveTo(M, y + ROW_H); ctx.lineTo(W - M, y + ROW_H); ctx.stroke();
            ...
            ctx.fillText('LIGA METROPOLITANA EJE ESTE · SAN MATEO, ARAGUA', W/2, H - 18);
            ...
            canvas.toBlob(async blob => {
                ...
                try {
                    if (navigator.canShare?.({ files: [file] })) {
                        await navigator.share({ files: [file], title: `Tabla ${categoria} · Liga Metropolitana Eje Este` });
                    } else {
            ...
            <div style={{ maxWidth: 860, margin: '0 auto', padding: '16px 16px 0' }}>
                {equiposConStats.length === 0 ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '60px 20px' }}>
                        ...
                        <p style={{ fontWeight: 900, color: '#475569', fontSize: '0.95rem', margin: '0 0 8px' }}>
                            Temporada no configurada
                        </p>
                        <p style={{ fontSize: '0.72rem', color: '#94a3b8', margin: 0 }}>
                            Usa Admin → ⚙️ Config para asignar los equipos a sus grupos antes de iniciar.
                        </p>
                    </div>
                ) : (
                    <>

  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 1. CARGA DE EQUIPOS Y MAPA DE CATEGORÍAS (Para aislamiento de mundos)
        ...
        equiposSnap.forEach(d => {
            ...
            if (n) {
                catMap[n] = data.categoria || 'SIN_CAT';
        ...
        setEquiposA(equiposDelMundo.filter(e => e.grupo === 'A' || e.grupo === 'a' || categoriaActiva === 'U19').sort((a,b) => b.puntos - a.puntos));
        setEquiposB(equiposDelMundo.filter(e => e.grupo === 'B' || e.grupo === 'b').sort((a,b) => b.puntos - a.puntos));

        // 2. CARGA DE CALENDARIO DEL MUNDO
        ...
        setProximosJuegos(allMatches.filter(m => m.estatus !== 'finalizado').reverse());

        // 3. LÍDERES FILTRADOS POR MUNDO (AISLAMIENTO TOTAL)
        ...
        statsSnap.forEach(docSnap => {
            ...
            const catReal = catMap[eqStat] || 'SIN_CAT';
            // Filtro estricto para evitar que Master 40 aparezca en otros mundos
            const pertenece = categoriaActiva === 'MASTER40' ? (catReal === 'MASTER40' || catReal === 'SIN_CAT') : (catReal === categoriaActiva);
            ...
            if (pertenece) {
                ...
                acc.bloq += (Number(stat.bloqueos)||0);
                acc.ast += (Number(stat.asistencias)||0);
                acc.pj += 1;
            }
        });
        ...
        const playerList = Object.values(aggregated).map((p) => {
            ...
            return { ...p, ppg: parseFloat((p.pts / den).toFixed(1)), rpg: parseFloat((p.reb / den).toFixed(1)), valpg: parseFloat(((p.pts + p.reb + p.rob + p.bloq) / den).toFixed(1)) };
        });

        if (playerList.length > 0) {
            setLeadersList([
                { label: 'MVP', p: [...playerList].sort((a,b) => b.valpg - a.valpg)[0], val: [...playerList].sort((a,b) => b.valpg - a.valpg)[0].valpg, unit: 'VAL', icon: '👑', color:'#eab308' },
      ...
      <header style={{ background: '#ffffff', padding: '15px 15px 10px 15px', borderBottom: '1px solid #f1f5f9', position: 'sticky', top: 0, zIndex: 1000 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom:'15px' }}>
          ...
          <div style={{ flex: 1, textAlign: 'right' }}>
             <div style={{ fontSize:'0.45rem', color:'#cbd5e1', fontWeight:'900', border:'1px dashed #cbd5e1', padding:'5px', borderRadius:'8px' }}>SPONSOR</div>
          </div>
        </div>

        <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '10px' }} className="no-scrollbar">
          {CATEGORIAS_DISPONIBLES.map(cat => (
            <button key={cat.id} onClick={() => setCategoriaActiva(cat.id)} style={{ padding: '8px 16px', borderRadius: '20px', border: 'none', whiteSpace: 'nowrap', background: categoriaActiva === cat.id ? '#1e3a8a' : '#f1f5f9', color: categoriaActiva === cat.id ? 'white' : '#64748b', fontSize: '0.65rem', fontWeight: '900' }}>{cat.label}</button>
          ))}
        </div>
      </header>
      ...
      <main style={{ padding: '15px', maxWidth: '500px', margin: '0 auto' }}>
        {activeView === 'login' ? (
           <div className="fade-in">
             <Login />
             <button onClick={() => setActiveView('dashboard')} style={{ width:'100%', marginTop:'20px', background:'none', border:'none', color:'#94a3b8', fontWeight:'bold', cursor:'pointer', fontSize:'0.7rem' }}>← VOLVER</button>
           </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
              ...
              <section>
                <h2 style={{ fontSize: '0.7rem', fontWeight: '900', color: '#1e3a8a', marginBottom: '10px', textTransform:'uppercase' }}>🏀 Resultados {categoriaActiva}</h2>
                    ...
                    <div key={juegoIndex} className="fade-in" style={{ height: '100%', background: 'linear-gradient(135deg, #1e3a8a, #1e40af)', color: 'white', padding: '20px', display:'flex', flexDirection:'column', justifyContent:'center' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                        <div style={{ textAlign: 'center', flex: 1 }}>
                          <div style={{ width:'50px', height:'50px', borderRadius:'50%', background:'white', margin:'0 auto', overflow:'hidden', border:'2px solid white' }}>
                            <img src={teamLogos[resultadosRecientes[juegoIndex].equipoLocalNombre?.trim().toUpperCase()]} style={{ width:'100%', height:'100%', objectFit:'contain' }} />
                          </div>
                        ...
                        <div style={{ textAlign: 'center', flex: 1 }}>
                          <div style={{ width:'50px', height:'50px', borderRadius:'50%', background:'white', margin:'0 auto', overflow:'hidden', border:'2px solid white' }}>
                            <img src={teamLogos[resultadosRecientes[juegoIndex].equipoVisitanteNombre?.trim().toUpperCase()]} style={{ width:'100%', height:'100%', objectFit:'contain' }} />
                          </div>
                          <p style={{ fontSize: '0.55rem', fontWeight: '900', marginTop: '5px' }}>{resultadosRecientes[juegoIndex].equipoVisitanteNombre}</p>
                        </div>
                      </div>
                      <button onClick={() => window.open('https://www.youtube.com/@barbakanzler', '_blank')} style={{ width: '100%', padding: '10px', borderRadius: '15px', border: 'none', background: '#f59e0b', color: 'white', fontWeight: '900', fontSize: '0.65rem', cursor: 'pointer' }}>▶ VER EN YOUTUBE</button>
                    </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                <div onClick={() => setActiveView('noticias')} style={{ height: '220px', background: 'white', borderRadius: '24px', border: '2.5px solid #1e3a8a', cursor: 'pointer', overflow:'hidden' }}>
                  ...
                  <div style={{ height: '110px', display:'flex', alignItems:'center', justifyContent:'center', background:'#f8fafc', padding:'5px' }}>
                    {noticias.length > 0 && <img key={noticiaIndex} src={noticias[noticiaIndex].imageUrl} className="fade-in" style={{ maxWidth: '100%', maxHeight:'100%', objectFit: 'contain' }} />}
                  </div>
                ...
                <div onClick={() => setActiveView('stats')} style={{ height: '220px', background: '#ffffff', borderRadius: '24px', border: `2.5px solid ${leadersList[leaderIndex]?.color || '#eee'}`, cursor: 'pointer', textAlign: 'center', display: 'flex', flexDirection:'column', position:'relative', overflow:'hidden' }}>
                  {leadersList.length > 0 ? (
                    <div key={leaderIndex} className="fade-in" style={{ height:'100%', display:'flex', flexDirection:'column' }}>
                      ...
                      <div style={{ flex: 1, display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', padding:'10px' }}>
                        <div style={{ width:'55px', height:'55px', borderRadius:'50%', background:'white', overflow:'hidden', border:`2.5px solid ${leadersList[leaderIndex].color}`, marginBottom:'8px' }}>
                          <img src={teamLogos[leadersList[leaderIndex].p?.equipo?.toUpperCase()] || ""} style={{ width:'100%', height:'100%', objectFit:'contain' }} />
                        </div>
                  ...
                  ) : <p style={{padding:'20px', fontSize:'0.6rem'}}>Cargando mundo...</p>}
                </div>
              </div>
              ...
              <section>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
                  <h2 style={{ fontSize: '0.75rem', fontWeight: '900', color: '#1e3a8a', margin: 0 }}>🏆 Clasificación</h2>
                ...
                <div onClick={() => setActiveView('tabla')} style={{ cursor: 'pointer' }}><RenderTableSummary title={`TABLA ${categoriaActiva}`} data={tablaIndex === 0 ? equiposA : equiposB} color={tablaIndex === 0 ? "#1e3a8a" : "#d97706"} /></div>
              </section>

              {/* PRÓXIMOS JUEGOS (NUEVA SECCIÓN) */}
              <section>
                <h2 style={{ fontSize: '0.75rem', fontWeight: '900', color: '#1e3a8a', marginBottom: '10px', textTransform:'uppercase' }}>📅 Próximos Juegos {categoriaActiva}</h2>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {proximosJuegos.length > 0 ? proximosJuegos.slice(0, 4).map(j => (
                    <div key={j.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '15px', background: '#f8fafc', borderRadius: '20px', border: '1.5px solid #e2e8f0' }}>
                      <div style={{ flex: 1, textAlign: 'right', fontSize: '0.7rem', fontWeight: '900' }}>{j.equipoLocalNombre.toUpperCase()}</div>
                      <div style={{ flex: 1, textAlign: 'center', margin: '0 10px' }}>
                        <span style={{ background: '#1e3a8a', color: 'white', padding: '4px 12px', borderRadius: '12px', fontSize: '0.6rem', fontWeight: '900' }}>{j.hora || 'VS'}</span>
                        <p style={{ fontSize: '0.5rem', color: '#94a3b8', marginTop: '5px' }}>{j.fechaAsignada}</p>
                      </div>
                      <div style={{ flex: 1, textAlign: 'left', fontSize: '0.7rem', fontWeight: '900' }}>{j.equipoVisitanteNombre.toUpperCase()}</div>
                    </div>
                  )) : <div style={{textAlign:'center', padding:'20px', background:'#f1f5f9', borderRadius:'20px', fontSize:'0.7rem', color:'#94a3b8'}}>Sin juegos programados</div>}
                </div>
              </section>

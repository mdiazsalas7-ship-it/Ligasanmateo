...
const MesaTecnica: React.FC<{ categoria: string; onClose: () => void }> = ({ categoria, onClose }) => {
    ...
    // Sincronizar enVivo cuando el partido está activo (DEBE ir después de colCal)
    useEffect(() => {
        if (!selectedMatchId || !colCal) return;
        if (startersDone) {
            updateDoc(doc(db, colCal, selectedMatchId), { enVivo: true }).catch(() => {});
        }
